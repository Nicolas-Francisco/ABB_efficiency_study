...
int main(){
    ...
    FILE* ficheroAleatorioBTree4096;
    ficheroAleatorioBTree4096 = fopen("Datos/aleatorioBTree4096.txt", "w");
    for (int i = 0; i < 100; i++){
        double timeA = aleatorio();
        fprintf(ficheroAleatorioBTree4096, "%f",timeA);
        if(i<99){
            fprintf(ficheroAleatorioBTree4096, ",");
        }        
    }
    fclose(ficheroAleatorioBTree4096);
    printf("\n");

    printf("------------------- Experimento creciente con factor 0.1 -------------------\n");
    FILE* ficheroCreciente_0p1_BTree4096;
    ficheroCreciente_0p1_BTree4096 = fopen("Datos/creciente_0p1_BTree4096.txt", "w");
    for (int i = 0; i < 100; i++){
        double timeC_0p1 = creciente(0.1);
        fprintf(ficheroCreciente_0p1_BTree4096, "%f",timeC_0p1);
        if(i<99){
            fprintf(ficheroCreciente_0p1_BTree4096, ",");
        }        
    }
    fclose(ficheroCreciente_0p1_BTree4096);
    printf("\n");

    printf("------------------- Experimento creciente con factor 0.5 -------------------\n");
    FILE* ficheroCreciente_0p5_BTree4096;
    ficheroCreciente_0p5_BTree4096 = fopen("Datos/creciente_0p5_BTree4096.txt", "w");
    for (int i = 0; i < 100; i++){
        double timeC_0p5 = creciente(0.5);
        fprintf(ficheroCreciente_0p5_BTree4096, "%f",timeC_0p5);
        if(i<99){
            fprintf(ficheroCreciente_0p5_BTree4096, ",");
        }        
    }
    fclose(ficheroCreciente_0p5_BTree4096);
    printf("\n");

    printf("------------------------------ Sesgado p(x)=x ------------------------------\n");
    FILE* ficheroSesgado_x_BTree4096;
    ficheroSesgado_x_BTree4096 = fopen("Datos/sesgado_x_BTree4096", "w");
    for (int i = 0; i < 100; i++){
        double time_s_x = sesgada(&p_x);
        fprintf(ficheroSesgado_x_BTree4096, "%f",time_s_x);
        if(i<99){
            fprintf(ficheroSesgado_x_BTree4096, ",");
        }        
    }
    fclose(ficheroSesgado_x_BTree4096);
    printf("\n");

    printf("--------------------------- Sesgado p(x)=sqrt(x) ---------------------------\n");
    
    FILE* ficheroSesgado_sqrt_BTree4096;
    ficheroSesgado_sqrt_BTree4096 = fopen("Datos/sesgado_sqrt_BTree4096", "w");
    for (int i = 0; i < 100; i++){
       double time_s_sqrt = sesgada(&p_sqrt);
        fprintf(ficheroSesgado_sqrt_BTree4096, "%f",time_s_sqrt);
        if(i<99){
            fprintf(ficheroSesgado_sqrt_BTree4096, ",");
        }        
    }
    fclose(ficheroSesgado_sqrt_BTree4096);
    printf("\n");

    printf("---------------------------- Sesgado p(x)=ln(x) ----------------------------\n");
    
    FILE* ficheroSesgado_ln_BTree4096;
    ficheroSesgado_ln_BTree4096 = fopen("Datos/sesgado_ln_BTree4096", "w");
    for (int i = 0; i < 100; i++){
       double time_s_ln = sesgada(&p_ln);
        fprintf(ficheroSesgado_ln_BTree4096, "%f",time_s_ln);
        if(i<99){
            fprintf(ficheroSesgado_ln_BTree4096, ",");
        }        
    }
    fclose(ficheroSesgado_ln_BTree4096);
    printf("\n");
}