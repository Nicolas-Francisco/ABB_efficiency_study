...
double aleatorio(){
    ...
    for (long i=0; i < n; i++){ //recorro mi secuencia
        ...
        if (operacion==0){ //la operacion es una insercion
            ...
            insert(random); //inserto el nodo en el arbol
            ...
            Node * node = search(random);//busco el nodo en mi arbol
...
int main(){
    ...
    FILE* ficheroAleatorioSpray;
    ficheroAleatorioSpray = fopen("Datos/aleatoriosplay.txt", "w");
    for (int i = 0; i < 100; i++){
        double timeA = aleatorio();
        fprintf(ficheroAleatorioSpray, "%f",timeA);
        if(i<99){
            fprintf(ficheroAleatorioSpray, ",");
        }        
    }
    fclose(ficheroAleatorioSpray);
    printf("\n");

    printf("--------------Experimento creciente con factor 0.1------------\n");
    FILE* ficheroCreciente_0p1_Spray;
    ficheroCreciente_0p1_Spray = fopen("Datos/creciente_0p1_splay.txt", "w");
    for (int i = 0; i < 100; i++){
        double timeC_0p1 = creciente(0.1);
        fprintf(ficheroCreciente_0p1_Spray, "%f",timeC_0p1);
        if(i<99){
            fprintf(ficheroCreciente_0p1_Spray, ",");
        }        
    }
    fclose(ficheroCreciente_0p1_Spray);
    printf("\n");

    printf("--------------Experimento creciente con factor 0.5------------\n");
    FILE* ficheroCreciente_0p5_Spray;
    ficheroCreciente_0p5_Spray = fopen("Datos/creciente_0p5_splay.txt", "w");
    for (int i = 0; i < 100; i++){
        double timeC_0p5 = creciente(0.5);
        fprintf(ficheroCreciente_0p5_Spray, "%f",timeC_0p5);
        if(i<99){
            fprintf(ficheroCreciente_0p5_Spray, ",");
        }        
    }
    fclose(ficheroCreciente_0p5_Spray);
    printf("\n");

    printf("--------------------Sesgado p(x)=x----------------------------\n");
    FILE* ficheroSesgado_x_Spray;
    ficheroSesgado_x_Spray = fopen("Datos/sesgado_x_splay.txt", "w");
    for (int i = 0; i < 100; i++){
        double time_s_x = sesgada(&p_x);
        fprintf(ficheroSesgado_x_Spray, "%f",time_s_x);
        if(i<99){
            fprintf(ficheroSesgado_x_Spray, ",");
        }        
    }
    fclose(ficheroSesgado_x_Spray);
    printf("\n");

    printf("--------------------Sesgado p(x)=sqrt(x)------------------------\n");
    
    FILE* ficheroSesgado_sqrt_Spray;
    ficheroSesgado_sqrt_Spray = fopen("Datos/sesgado_sqrt_splay.txt", "w");
    for (int i = 0; i < 100; i++){
       double time_s_sqrt = sesgada(&p_sqrt);
        fprintf(ficheroSesgado_sqrt_Spray, "%f",time_s_sqrt);
        if(i<99){
            fprintf(ficheroSesgado_sqrt_Spray, ",");
        }        
    }
    fclose(ficheroSesgado_sqrt_Spray);
    printf("\n");

    printf("--------------------Sesgado p(x)=ln(x)---------------------------\n");
    
    FILE* ficheroSesgado_ln_Spray;
    ficheroSesgado_ln_Spray = fopen("Datos/sesgado_ln_splay.txt", "w");
    for (int i = 0; i < 100; i++){
       double time_s_ln = sesgada(&p_ln);
        fprintf(ficheroSesgado_ln_Spray, "%f",time_s_ln);
        if(i<99){
            fprintf(ficheroSesgado_ln_Spray, ",");
        }        
    }
    fclose(ficheroSesgado_ln_Spray);
    printf("\n");
